...
from LPTS.settings import API_KEY, API_URL
import time



@login_required(login_url='users:login')
def vuln_scan_index(request):
    ...
    for Middleware in Middleware_datas:
        print(Middleware.url, Middleware.CVE_id, Middleware.result,
              time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(float(Middleware.time))))
...
def get_vuln_rank(request):
    ...
    datas = json.loads(d.stats())["top_vulnerabilities"]
    vuln_rank = []
    i = 0
    for data in datas:
        if data:
            item = {}
            item['name'] = data['name']
            item['value'] = data['count']
            vuln_rank.append(item)
        else:
            break
        i += 1
        if i >=5:
